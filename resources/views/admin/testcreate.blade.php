    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="panel-heading">Create a test question</div>
                <div class="panel-body">
                    <form class="form-horizontal" role="form" method="POST" action="{{ route('test.store') }}">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group{{ $errors->has('min_point') ? ' has-error' : '' }}">
                            <label for="exercise_id" class="col-md-4 control-label">Id bài tập</label>

                            <div class="col-md-6">
                                <select class="form-control" name="exercise_id" id="exercise_id">
                                    @foreach($exericses as $exericse)
                                        <option value="{{$exericse->id}}"?>{{$exericse->id}}</option>
                                    @endforeach\
                                </select>
                                <br/>
